# streamlit_app.py — Gestão da Turma de Muay Thai (SQLite local ou Postgres/Neon)
# Versão: v2.14.0 (cloud-ready)
...
# -----------------------------------------------
# Banco de dados: 1) variável de ambiente, 2) secrets (se existir), 3) SQLite
# -----------------------------------------------
def _get_db_url() -> str:
...
# Info visual de banco
try:
    st.caption("Banco: " + ("Postgres/Neon" if DB_URL.startswith("postgresql") else "SQLite local"))
except Exception:
    pass

# Evitar redefinição de tabelas em recarregamentos
...
class Settings(SQLModel, table=True):
    ...
    default_master_percent: float = Field(default=0.20)  # 20%
...
class Coach(SQLModel, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True)
    name: str
    full_pass: bool = Field(default=False)  # repasse 100%
...
class Student(SQLModel, table=True):
    ...
    master_percent_override: Optional[float] = Field(default=None)  # 0.0–1.0
...
class Payment(SQLModel, table=True):
    ...
    notes: Optional[str] = Field(default=None)
    master_percent_used: float = Field(default=0.0)     # 0.0–1.0
    master_adjustment: float = Field(default=0.0)
    master_amount: float = Field(default=0.0)

# -----------------------------------------------
# DB Init & Migração (apenas SQLite legado)
# -----------------------------------------------
def init_db():
    if IS_SQLITE:
        # Limpeza defensiva de índices antigos que causavam conflitos
            ...
            try:
                ...
                for (idx_name,) in rows:
                    if idx_name.startswith(("ix_", "idx_", "sqlite_autoindex")):
                        try: conn.exec_driver_sql(f'DROP INDEX IF EXISTS "{idx_name}"')
...
def ensure_coach_full_pass_column_sqlite():
    if not IS_SQLITE:
        return
    try:
        with engine.begin() as conn:
...
def update_student(
    ...
    coach_id: int | None = None,
    train_slot_id: int | None = None,
    master_percent: float | None = None,  # compat
    master_percent_override: float | None = None,
    grade: str | None = None,
...
def delete_student(student_id: int) -> int:
    with Session(engine) as s:
        ...
        if not obj: return 0
        s.delete(obj); s.commit(); return 1
...
def record_payment_batch(student_ids: list, paid_date_val: date, amount_common: Optional[float],
    ...
    for sid in student_ids:
        ...
        # Repasse 100% se professor marcado
        coach_full = False
        try:
            if s.coach_id:
                with Session(engine) as session:
    ...
    with Session(engine) as session:
        ...
        for k,v in fields.items():
            if hasattr(obj, k) and v is not None:
                setattr(obj, k, v)
        session.add(obj); session.commit(); return 1
...
def tempo_meses(student):
    if not student or not getattr(student, "start_date", None):
        return 0
    start = student.start_date; today = date.today()
...
def get_students(only_active: bool = True):
    with Session(engine) as s:
        q = select(Student)
        if only_active: q = q.where(Student.active == True)
...
def get_coaches():
    with Session(engine) as s:
        try:
            ...
            try: ensure_coach_full_pass_column_sqlite()
            except Exception: pass
            stmt = select(Coach).order_by(Coach.name)
            return list(s.exec(stmt))
...
def money(x: float, sym: str) -> str:
    try: return f"{sym} {float(x):,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
    except Exception: return f"{sym} 0,00"

def fmt_percent(dec):
    if dec is None: return ""
    try: return f"{float(dec)*100:.0f}%"
...
# -----------------------------------------------
init_db()
migrate_sqlite_legacy()
ensure_coach_full_pass_column_sqlite()
cfg = get_settings()

with st.sidebar:
    st.caption("Versão: v2.14.0 (Cloud ready)")
    page = st.radio("Navegar para:", [
...
# -----------------------------------------------
    ...
    if not students:
        ...
        if not gdf.empty:
            try:
                ...
                if st.button("🗑️ Excluir selecionadas", use_container_width=True, key="btn_del_grads_page"):
                    ...
                    st.success(f"{cnt} graduação(ões) excluída(s)."); st.rerun()
            except Exception:
                pass
        ...
        if st.button("➕ Adicionar graduação", use_container_width=True):
            ...
            st.success("Graduação adicionada!"); st.rerun()
        ...
        with col3:
            ...
            if st.button("✅ Confirmar pagamento", use_container_width=True, key="btn_conf_pay"):
                if not chosen_ids:
                    ...
                    st.success(f"{n} pagamento(s) registrado(s)."); st.rerun()
            ...
            with colx:
                if st.button("🗑️ Excluir selecionados", use_container_width=True, key="btn_del_sel"):
                    if ids_to_delete:
                        ...
                        st.success(f"{n} pagamento(s) excluído(s)."); st.rerun()
                    else:
            ...
            with coly:
                if st.button("🧹 Excluir todos deste mês", use_container_width=True, key="btn_del_all_month"):
                    ...
                    if all_ids:
                        ...
                        st.success(f"Todos os {n} pagamentos deste mês foram excluídos."); st.rerun()
                    else:
...
# -----------------------------------------------
    ...
    if st.button("💾 Salvar extra", use_container_width=True):
        if not e_desc.strip():
            ...
            st.success("Extra lançado."); st.rerun()
    ...
    if dfe is None or dfe.empty:
        ...
        with cdx1:
            if st.button("🗑️ Excluir selecionados", use_container_width=True):
                if dels:
                    n = delete_extra_repasse(dels); st.success(f"{n} lançamento(s) excluído(s)."); st.rerun()
                else:
                    st.warning("Nenhum lançamento selecionado.")
        with cdx2:
            if st.button("🧹 Excluir todos listados", use_container_width=True):
                n = delete_extra_repasse(dfe["id"].tolist()); st.success(f"Todos os {n} lançamentos listados foram excluídos."); st.rerun()
...
# -----------------------------------------------
    ...
    <style>
    :root{
      --card-bg:#ffffff; --card-text:#111827; --card-muted:#6b7280; --card-border:#e5e7eb;
    }
    @media (prefers-color-scheme: dark){
      :root{ --card-bg:#111827; --card-text:#f9fafb; --card-muted:#9ca3af; --card-border:#374151; }
    }
    .card{border-radius:12px;padding:16px 18px;background:var(--card-bg);color:var(--card-text);border:1px solid var(--card-border);box-shadow:0 2px 8px rgba(0,0,0,.04);text-align:center}
    ...
    if dfp is not None and not dfp.empty:
        ...
        if not pag.empty:
            ...
            st.info("Sem pagamentos para o filtro atual."); total_pagamentos_repasse = 0.0
    else:
        st.info("Sem pagamentos neste mês."); total_pagamentos_repasse = 0.0
    ...
    if dfe is not None and not dfe.empty:
        ...
        st.info("Sem extras neste mês."); total_extras = 0.0
...
# -----------------------------------------------
    ...
    if st.button("➕ Adicionar professor"):
        if not new_coach.strip(): st.warning("Informe um nome.")
            ...
            add_coach(new_coach.strip(), full_pass=bool(new_full)); st.success("Professor adicionado!")
    clist = list_coaches()
    if clist:
        for c in clist:
